...
func _physics_process(delta: float) -> void:
	...
	if is_instance_valid(_sprite) and dir != 0.0:
...
func _perform_jump() -> void:
	...
	if was_double and is_instance_valid(_sprite):
		var tween: Tween = create_tween()
		tween.set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
		tween.tween_property(_sprite, "scale", Vector2(1.4, 0.6), 0.07)
		tween.tween_property(_sprite, "scale", Vector2(1.0, 1.0), 0.18)
...
func die() -> void:
	...
	if is_instance_valid(_sprite):
		...
		tween.tween_property(_sprite, "modulate", Color(1.0, 0.15, 0.0, 0.0), 0.5)
		tween.tween_property(_sprite, "scale", Vector2(1.6, 1.6), 0.5)
...
func _respawn() -> void:
	...
	if is_instance_valid(_sprite):
		...
		var tween: Tween = create_tween()
		tween.tween_property(_sprite, "modulate:a", 1.0, 0.35)

# ---------------------------------------------------------------------------
# Skin — modulate tints the whole sprite; shirt is light-coloured in SVG
# so the skin colour shows clearly there while dark areas stay subtle
# ---------------------------------------------------------------------------
func apply_skin(skin_id: String) -> void:
	if is_instance_valid(_sprite):
		_sprite.modulate = ShopSystem.get_skin_color(skin_id)
